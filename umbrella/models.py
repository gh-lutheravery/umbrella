...
class Post(DBModel):
    ...
    def get_post(self, post_filter=None):
        if post_filter:
    ...
    def query_posts(self, post_filter=None, ind_flag=False):
        if post_filter:
            if ind_flag:
                return self.get_post(post_filter)

            rows = read_rows(self.table_name)
            posts = []
            for r in rows:
                post = self.get_post(post_filter)
                posts.append(post)
